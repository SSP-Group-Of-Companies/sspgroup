...
export function AboutSspMissionValuesSection({ data }: { data: MissionVisionContent }) {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden scroll-mt-24 border-b border-[color:var(--color-border-light-soft)] py-20 sm:py-24 lg:py-28"
      style={{
        background:
          "linear-gradient(180deg, var(--color-surface-1-light) 0%, var(--color-surface-0-light) 58%, var(--color-ssp-cloud-50) 100%)",
      }}
    >
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div
          className="absolute inset-x-0 top-0 h-20"
          style={{ background: "linear-gradient(180deg, rgba(7,16,30,0.05) 0%, transparent 100%)" }}
        />
        <div
          className="absolute -left-16 top-[12%] h-[220px] w-[220px] rounded-full bg-[color:var(--color-ssp-cyan-500)]/[0.05] blur-[115px]"
        />
        <div
          className="absolute -right-20 bottom-[8%] h-[230px] w-[230px] rounded-full bg-[color:var(--color-brand-500)]/[0.04] blur-[115px]"
        />
      </div>
      ...
      <Container className="site-page-container relative">
        ...
        >
          ...
          <motion.h2
            ...
            className="mt-4 text-3xl leading-tight font-bold tracking-tight text-[color:var(--color-text-strong)] text-pretty sm:text-4xl lg:text-[2.75rem]"
          >
            {data.sectionTitle}
          </motion.h2>
          <motion.p
            variants={revealUp}
            transition={{ duration: reduceMotion ? 0 : 0.35, ease: "easeOut" }}
            className="mt-3 max-w-3xl text-[15px] leading-8 text-[color:var(--color-muted)]"
          >
        ...
        >
          <motion.article
            ...
            className={cn(
              CARD_BASE,
              "flex flex-col border border-[color:var(--color-menu-border)]/80 bg-[linear-gradient(145deg,var(--color-company-hero-midnight-start),var(--color-company-ink)_62%,var(--color-company-hero-midnight-end))] shadow-[0_22px_46px_-30px_rgba(2,6,23,0.74)] lg:col-span-7",
            )}
          >
            ...
            <h3
              className={cn(
                CARD_TITLE,
                "mt-3 max-w-[22ch] text-white text-pretty sm:max-w-none",
              )}
            >
              {data.missionTitle}
            </h3>
          ...
          <motion.article
            ...
            className={cn(
              CARD_BASE,
              "flex flex-col border border-[color:var(--color-border-light)] bg-white shadow-[0_20px_44px_-30px_rgba(2,6,23,0.28)] lg:col-span-5",
            )}
          >
        ...
        >
          ...
          >
            <h3
              ...
              className="text-2xl font-bold leading-tight tracking-tight text-[color:var(--color-text-strong)] text-pretty sm:text-3xl lg:text-[2rem]"
            >
              {data.valuesTitle}
            </h3>
            <p className="mt-3 max-w-3xl text-[15px] leading-8 text-[color:var(--color-muted)]">
              {data.valuesSubtitle}
            </p>
          </motion.div>
          ...
          {data.principles.map((principle, idx) => (
            <motion.article
              ...
              className={cn(
                ...
                idx >= 1 &&
                  "max-sm:border-t max-sm:border-[color:var(--color-border-light-soft)] max-sm:pt-10",
            ...
            >
              <span
                className="select-none font-semibold tabular-nums tracking-tight text-[2.15rem] leading-none text-[color:var(--color-subtle)]/30 transition-colors duration-300 group-hover:text-[color:var(--color-subtle)]/50 sm:text-[2.35rem]"
                aria-hidden
              >
                {String(idx + 1).padStart(2, "0")}
              </span>
              <h4 className="mt-5 max-w-[20ch] text-[0.98rem] font-semibold leading-snug tracking-tight text-[color:var(--color-text-strong)] text-pretty sm:mt-6 sm:max-w-none sm:text-[1.05rem]">
                {principle.title}
              </h4>
              <p className="mt-3 max-w-prose flex-1 text-[14px] leading-[1.82] text-[color:var(--color-muted)] sm:mt-4 sm:text-[15px] sm:leading-[1.78]">
                {principle.body}
              </p>
            </motion.article>
          ))}
